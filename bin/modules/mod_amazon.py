...
def main():
    ...
    for item in items['amazon']:
        ...
        while price == "" and n_tries <= int(scraping_max_tries):
            ...
            try:
                price = soup.find("span", attrs={'id':'priceblock_ourprice'}).string.strip()[:-2].replace(".", "").replace(",", ".")
                break
            except AttributeError:
                try:
                    # If there is some deal price
                    price = soup.find("span", attrs={'id':'priceblock_dealprice'}).string.strip()[:-2].replace(".", "").replace(",", ".")
                    break
                except AttributeError:
                    try:
                        # If there is some deal price
                        price = soup.find("span", attrs={'id':'priceblock_saleprice'}).string.strip()[:-2].replace(".", "").replace(",", ".")
                        break
                    except:
                        price = ""
            n_tries += 1
